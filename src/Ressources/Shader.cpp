  ...
  : Ressource("Shader", name)
{
  ...
  viewPosId = glGetUniformLocation(progId, "viewPos");
}
...
void Shader::SetM(const float * m)
{
	if (m) glUniformMatrix4fv(mId, 1, GL_TRUE, m);
}

void Shader::SetCam(const Matrix4 & pos)
{
	glUniform3f(viewPosId, pos.m[3], pos.m[7], pos.m[11]);
}
