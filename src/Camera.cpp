...
void Camera::UsePerspectiveMatrix(float n, float f) {
	near = n;
	far = f;
...
void Camera::UseOrthoMatrix(float left, float right, float bottom, float top, float n, float f)
{
	near = n;
	far = f;
	...
	projection.m[3] = tx;
	projection.m[4] = 0.0f;
	projection.m[5] = 2 / (top - bottom);
	projection.m[6] = 0.0f;
	projection.m[7] = ty;
	projection.m[8] = 0.0f;
	projection.m[9] = 0.0f;
	projection.m[10] = -2 / (far - near);
	projection.m[11] = tz;
	projection.m[12] = 0.0f;
	projection.m[13] = 0.0f;
